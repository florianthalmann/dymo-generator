function DymoGenerator(a,d){function e(a,n){var b=k.addDymo(a),c=n.getFeatures(),d;for(d in c)k.setDymoFeature(b,h(d),c[d]);k.setDymoFeature(b,h("level"),n.getLevel());c=n.getParts();for(d=0;d<c.length;d++)e(b,c[d])}function b(a){r=p.toJsonHierarchyGraph();u=p.toJsonSimilarityGraph();if(a){var b=a.toFlatJson();q[a.getUri()]=a;n[a.getUri()]=b;for(b=0;b<v.length;b++)c(a,v[b])}}function c(a,n){var b=a.getFeature(n.name);if(!isNaN(b))f(n,b);else if(b instanceof Array)for(var c=0;c<b.length;c++)f(n,b[c])}
...
0;b<v.length;b++)if(v[b].name==a)return v[b];a=l(a);v.splice(v.length-2,0,a);d(a);return a}function l(a,b,c){return void 0!=b&&void 0!=c?{name:a,min:b,max:c}:{name:a,min:1E3,max:0}}var k=this,p,m,t,r,u,q,n,v,x,w=MEAN,y;this.resetDymo=function(){m=p=void 0;t=!1;r={nodes:[],links:[]};u={nodes:[],links:[]};q={};n={};v=[l("level"),l("random",0,1)];x=0};this.setDymo=function(a,b){this.resetDymo();e(void 0,a)};this.getDymo=function(){return p};this.getDymoGraph=function(){return r};this.getSimilarityGraph=
function(){return u};this.getFeatures=function(){return v};this.setCondensationMode=function(a){w=a};this.setCurrentSourcePath=function(a){y=a};this.getScheduler=function(){return a};this.getRealDymo=function(a){return q[a["@id"]]};this.setAudioFileChanged=function(){t=!0;if(p){var c=new DynamicMusicObject("dymo"+Object.keys(q).length,a,PARALLEL);c.addPart(p);p=c;b(p)}};this.addDymo=function(c,n){c&&c.getUri();var d=new DynamicMusicObject("dymo"+Object.keys(q).length,a);p||(p=d);c&&c.addPart(d);n&&
d.setSourcePath(n);b(d);return d};this.addFeature=function(a,c,n){a=h(a);for(n=0;n<r.nodes.length;n++){var d=r.nodes[n].time.value,e=r.nodes[n].duration.value,f=c.filter(function(a){return d<=a.time.value&&a.time.value<d+e});1>f.length&&(f=c.filter(function(a){return a.time.value<d}),f=f[f.length-1]);f=g(f);this.setDymoFeature(this.getRealDymo(r.nodes[n]),a,f)}b()};this.addSegmentation=function(a){0==Object.keys(q).length?m=this.addDymo(void 0,y):t&&(m=this.addDymo(p,y),x=m.getLevel(),t=!1);for(var c=
...
b[c]);var d=b[c].split("_"),g=Number.parseInt(d[4].substring(1)),h=Number.parseInt(d[6].substring(2))/1E3,l=Number.parseInt(d[7].substring(2))/1E3,k=Number.parseInt(d[8].substring(2)),p=Number.parseInt(d[9].substring(2))/1E3,d=Number.parseInt(d[10].substring(2))/1E3,m=a.addDymo(e,"audio/Chopin_Op028-11_003_20100611-SMD-cut/"+b[c]);a.setDymoFeature(m,PITCH_FEATURE,g);a.setDymoFeature(m,ONSET_FEATURE,h);a.setDymoFeature(m,DURATION_FEATURE,l);a.setDymoFeature(m,"velocity",k);a.setDymoFeature(m,"onsetS",
p);a.setDymoFeature(m,"durationS",d);m.getParameter(ONSET).update(h)}e.updatePartOrder(ONSET_FEATURE)})};
DymoTemplates.createAchBachDymo=function(){var a=getFeature(ONSET_FEATURE),d=getFeature(PITCH_FEATURE),e=getFeature(DURATION_FEATURE),b=getFeature("onsetS"),c=getFeature("durationS"),f=getFeature("time"),g=addDymo();setDymoFeature(g,a,0);setDymoFeature(g,d,0);setDymoFeature(g,e,0);$http.get("audio/achachbach10/01-AchGottundHerr.txt").success(function(h){h=h.split("\n");for(var l=0;l<h.length;l++)h[l]=h[l].split("\t");h.sort(function(a,c){return a[0]-c[0]});for(var k=[],l=0;l<h.length;l++){var p=h[l][0],
m=h[l][1],t=Number.parseInt(h[l][3]);if(k[t]){var r=k[t][0],u=m-k[t][2];h[r][4]=p-k[t][1];h[r][5]=u}k[t]=[l,p,m]}for(l=h.length-4;l<h.length;l++)h[l][4]=2700,h[l][5]=2700;for(l=0;l<h.length;l++)if(6==h[l].length){var k=h[l],p=Number.parseInt(k[2]),m=Number.parseInt(k[0])/1E3,t=Number.parseInt(k[1])/1E3,r=Number.parseInt(k[4])/1E3,u=Number.parseInt(k[5])/1E3,q;1==k[3]?q=addDymo(g,"audio/achachbach10/01-AchGottundHerr-violin.wav"):2==k[3]?q=addDymo(g,"audio/achachbach10/01-AchGottundHerr-clarinet.wav"):
3==k[3]?q=addDymo(g,"audio/achachbach10/01-AchGottundHerr-saxphone.wav"):4==k[3]&&(q=addDymo(g,"audio/achachbach10/01-AchGottundHerr-bassoon.wav"));setDymoFeature(q,d,p);setDymoFeature(q,f,m);setDymoFeature(q,a,m);setDymoFeature(q,e,r);setDymoFeature(q,b,t);setDymoFeature(q,c,u);q.getParameter(ONSET).update(m)}g.updatePartOrder(a.name)});$http.get("getsourcefilesindir/",{params:{directory:"audio/achachbach10/"}}).success(function(a){a=a.filter(function(a){return 0<=a.indexOf("wav")});for(var c=0;c<
...
"P"==a.charAt(0)&&(a=a.substring(2,a.length-1));return Number(a)}function h(a,b,c,d){q[a]=d.sort(function(a,b){return a.time.value-b.time.value});d=q[a];b&&q[a][0].label&&(d=d.filter(function(a){return a.label.value==b}));c.addSegmentation(d)}function l(a,b,c,d){u(a,function(a){k(JSON.parse(a),b,c,d)})}function k(a,b,c,d){"file_metadata"==Object.keys(a)[0]?p(a,b,c,d):m(a,b,c,d)}function p(a,b,c,d){a=a[Object.keys(a)[1]][0];var e=a.annotation_metadata.annotator.output_id;"beats"==e||"onsets"==e?(a=
